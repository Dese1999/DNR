...
def start_KE(cfg):
    ...
    # Dictionary to store mask history for multiple layers across generations
    mask_history = {}  # اضافه شده برای ذخیره ماسک‌ها
    ...
    for gen in range(cfg.num_generations):
        ...
        # Store masks for each layer in each generation
        mask_history[gen] = {}
        if sparse_mask is not None:  # فقط اگر ماسک وجود داشته باشد
    ...
    # Plot mask sparsity changes across generations
    if mask_history and len(mask_history) > 0:  # اگر ماسک‌ها وجود داشته باشند
        plt.figure(figsize=(15, 10))
        # فرض می‌کنیم همه نسل‌ها لایه‌های یکسانی دارند، از نسل اول برای نام لایه‌ها استفاده می‌کنیم
        for layer_name in mask_history[0].keys():
            sparsity_per_gen = []
            for gen in range(cfg.num_generations):
                if gen in mask_history and layer_name in mask_history[gen]:  # بررسی وجود ماسک
                    mask = mask_history[gen][layer_name]
                    sparsity = 100 * (1 - mask.mean())  # درصد اسپارسیتی (0ها)
                    sparsity_per_gen.append(sparsity)
                else:
                    sparsity_per_gen.append(0)  # اگر ماسکی نبود، صفر در نظر می‌گیریم
